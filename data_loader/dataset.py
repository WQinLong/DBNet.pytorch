...
from utils import order_points_clockwise, get_datalist, load, expand_polygon, order_points_clockwise_list


class LSVTDataset(BaseDataSet):
    ...
    def _get_annotation(self, label_path: str) -> dict:
        ...
        with open(label_path, encoding='utf-8', mode='r') as f:
            for line in f.readlines():
                labels = line.strip().strip('\ufeff').strip('\xef\xbb\xbf').split('%^&*')
                if len(labels) != 2:
                    print('load label failed on %^&* {}'.format(label_path))
                    continue
                params = labels[0].split(',')
                label = labels[1]
                # params = line.strip().strip('\ufeff').strip('\xef\xbb\xbf').split(',')
                try:
                    # box = order_points_clockwise(np.array(list(map(float, params[:-1]))).reshape(-1, 2))
                    # if len(params) != 8:
                    #     print("")
                    # box = order_points_clockwise(np.array(list(map(float, params))).reshape(-1, 2))
                    box = np.array(list(map(float, params))).reshape(-1, 2)
